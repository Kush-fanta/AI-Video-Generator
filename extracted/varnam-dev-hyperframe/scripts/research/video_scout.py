#!/usr/bin/env python3
# ---
# varnam_script: research.video_scout
# owner: researcher
# status: live
# surface: direct-only
# purpose: Video metadata and transcript scouting.
# use_when: Inspect metadata, transcripts, and useful segments before download.
...
def require_dependency(dependency: object, install_hint: str) -> None:
    if dependency is None:
        raise RuntimeError(f"Missing dependency. Install it with `{install_hint}`.")
...
def get_video_info(url: str) -> dict:
    ...
    require_dependency(yt_dlp, "pip install yt-dlp")
...
def get_transcript(url: str, languages: list = None) -> list:
    ...
    require_dependency(YouTubeTranscriptApi, "pip install youtube-transcript-api")
